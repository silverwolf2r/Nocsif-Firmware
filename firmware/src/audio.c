/*
 * NocSif — audio output (MAX98357A I2S speaker) worker (M11 watch-core, slice E1·1). See audio.h.
 *
 * Architecture (mirrors wifi.c / imu.c):
 *   - A dedicated worker task owns every blocking action — the I2S writes and the BLDO2 amp rail.
 *     LVGL callbacks only post a tone/cue command to the worker's queue, so LVGL never blocks on
 *     audio and the I2S channel has a single owner (no cross-task races).
 *   - Bring-up is LAZY: the I2S standard-mode TX channel is allocated + configured on the first
 *     play (i2s_lazy_open), so boot stays light. The amp rail (AXP2101 BLDO2) is powered only while
 *     a sound is playing and dropped in between, so idle draw is low.
 *
 * Signal path: tones are synthesized on the worker as 16-bit signed PCM (sine), duplicated to both
 * I2S slots (stereo frame, L==R) so the MAX98357A produces sound regardless of how its channel-select
 * pin is strapped. A short linear fade-in/out on each segment tames the on/off click. The amp is mono,
 * so a 16 kHz sample rate is plenty for chimes and alert tones.
 */
#include "audio.h"
...
#include <stdio.h>          /* fopen/fread — voice-memo WAV playback */
...
/* minimp3 (components/minimp3, CC0): the ONE translation unit that carries the implementation. Xtensa has
 * no SIMD; MP3-only keeps the code small. Decoder state + buffers are PSRAM heap; the ~19 KB of decode
 * scratch lives on this worker's PSRAM stack (sized below). */
#define MINIMP3_IMPLEMENTATION
#define MINIMP3_ONLY_MP3
#define MINIMP3_NO_SIMD
#include "minimp3.h"

#include "power.h"          /* nocsif_power_speaker_rail — BLDO2 amp rail */
#include "settings.h"       /* mute / volume / boot+usb sound persistence */
#include "sdcard.h"         /* nocsif_sdcard_lock/unlock — /sd read for WAV playback */
#include "usb_gadget.h"     /* nocsif_usb_gadget_claim_sd — own /sd away from USB-MSC while reading */

static const char *TAG = "audio";

/* ---- hardware / tunables ---------------------------------------------------------- */
#define AUD_I2S_PORT      I2S_NUM_1     /* the amp path (I2S_NUM_0 is reserved for the PDM mic, E1·2) */
#define AUD_BCLK_GPIO     9             /* MAX98357A BCLK  (lilygo_twatch_ultra pins_arduino.h)       */
...
#define AUD_RAIL_SETTLE_MS 8            /* let BLDO2 settle before clocking the amp                    */
#define AUD_MAX_MS        2000          /* per-segment duration ceiling (guards a bad caller)          */
#define AUD_CMD_QLEN      6             /* queued cues/tones before the oldest is dropped             */
#define AUD_MAX_SEG       4             /* tone segments in one cue (played back-to-back, rail up)     */
#define AUD_CHUNK_FRAMES  256           /* synth/write chunk (stereo int16 -> 1 KB scratch)           */
#define AUD_VOL_DEFAULT   255           /* master speaker volume 0..255 (default full = no cue change) */
#define AUD_WAV_MAX_BYTES (2u*1024*1024)/* cap a WAV read into PSRAM (our memos are < 1 MB)           */
#define AUD_PATH_MAX      80
/* §4.13 fix #5: voice-memo playback make-up gain. mic.c peak-normalizes each recording to ~0.9 FS, but
 * speech is peaky so its RMS (perceived loudness) stays low -> memos play back too quiet. Apply a make-up
 * gain > 1 on the WAV path ONLY (cues are already ~0.9 FS), then a soft-knee limiter so a boosted peak is
 * compressed rather than wrapping sign into a crackle (a hard saturating clamp backstops it). Scoped to
 * WAV so cue/boot tones are untouched. AUD_WAV_MAKEUP is the loudness lever; AUD_WAV_KNEE is where the
 * limiter starts (fraction of full-scale). */
 ...
 * (1..8 B), and the accepted sample-rate window (the I2S clock follows the file). */
#define AUD_FILE_BLOCK    16384
#define AUD_FILE_RATE_MIN 8000
#define AUD_FILE_RATE_MAX 48000
#define AUD_MEMO_MAKEUP_X100 240        /* nocsif_audio_play_wav: the E1·3 voice-memo make-up (2.4x) */
...
/* A queued sound is EITHER a tone cue (up to AUD_MAX_SEG back-to-back segments, played with the amp
 * rail up for the whole command) OR a request to play a WAV file from /sd. */
...
typedef struct {
    ...
    char     path[AUD_PATH_MAX];  /* AUD_CMD_WAV: the /sd WAV path        */
} audio_cmd_t;

static TaskHandle_t        s_task;
static QueueHandle_t       s_q;
static i2s_chan_handle_t   s_tx;                     /* NULL until first play (lazy)          */
static volatile bool       s_tx_failed;              /* boot reserve tried + failed: cue is honestly dead */
...
static volatile bool       s_playing;                /* a WAV is currently playing            */
...
/* Phase B file engine state. s_stop_req is set by any task (Stop, or a play request that pre-empts the
 * current file) and consumed by the worker at the top of each chunk; the playing path is a spinlock-
 * guarded copy for the UI's "now playing" line. s_cur_rate tracks the I2S clock (worker-only). */
...
/* ---- cue table -------------------------------------------------------------------- *
 * Frequencies are round musical-ish pitches; durations short so a cue never feels laggy. */
...
/* Allocate + configure the standard-mode TX channel the first time we play. Leaves the channel
...
static esp_err_t i2s_lazy_open(void)
{
    ...
    if (s_tx_failed) {
        /* A prior attempt (the boot reserve, or an earlier open) could not claim the ~5.7 KB of
         * internal-DMA descriptors. Runtime teardown never defragments the pool back, so re-attempting
         * only re-logs NO_MEM on every beep — return the cached failure silently. */
...
/* Match the I2S clock to a sample rate (Phase B: the clock follows each file; tones/cues restore
 * AUD_SAMPLE_RATE the same way). The channel must be DISABLED — it always is between plays. The DMA
 * descriptors are rate-independent, so this touches no internal memory. */
...
static void play_segment(uint16_t freq, uint16_t ms, uint8_t vol)
{
    ...
    uint32_t total   = (uint32_t)AUD_SAMPLE_RATE * ms / 1000;   /* frames to emit */
    uint32_t fade    = AUD_SAMPLE_RATE * 4 / 1000;              /* ~4 ms edge ramp */
    if (fade > total / 2) {
        fade = total / 2;
    }
    float    amp     = (vol > 100 ? 100 : vol) / 100.0f * 0.9f * 32767.0f;  /* near full-scale (louder) */
    double   phase   = 0.0;
    ...
    while (done < total) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            uint32_t g = done + i;                              /* global frame index */
            float env = 1.0f;
            if (g < fade) {
                env = (float)g / (float)fade;                   /* fade in  */
            } else if (g >= total - fade) {
                env = (float)(total - g) / (float)fade;         /* fade out */
            }
            ...
            s_chunk[i * 2]     = s;                              /* L */
            s_chunk[i * 2 + 1] = s;                              /* R (duplicate — amp channel-agnostic) */
        }
        size_t wrote = 0;
...
/* Play one queued command: power the amp, clock out every segment, then drop the amp. */
static void do_play(const audio_cmd_t *c)
{
    if (i2s_lazy_open() != ESP_OK) {
        return;
    }
    ensure_rate(AUD_SAMPLE_RATE);           /* a file may have left the clock elsewhere */
    ...
    /* A tiny tail of silence flushes the last DMA frames before we cut the clock/rail (avoids a
     * clipped final sample / tail click). */
    play_segment(0, 6, 0);
 ...
 * downmixed to mono float, scaled (master volume x make-up), soft-knee limited (§4.13 fix #5), and written
...
/* Walk the RIFF chunks (caller holds the /sd lock). Leaves the stream positioned at the sample data. */
static bool wav_parse(FILE *f, wav_info_t *w)
{
    ...
    memset(w, 0, sizeof *w);
    bool have_fmt = false;
    for (int guard = 0; guard < 32; guard++) {          /* bounded: a hostile file cannot spin us */
        uint8_t ch[8];
        ...
        if (memcmp(ch, "fmt ", 4) == 0) {
            ...
            w->block = le16(fm + 12);
            w->bits  = le16(fm + 14);
            if (w->fmt == 0xFFFE && take >= 26) w->fmt = le16(fm + 24);   /* EXTENSIBLE: the sub-format tag */
            have_fmt = true;
        } else if (memcmp(ch, "data", 4) == 0) {
            w->data_off = ftell(f);
            w->data_len = sz;
            break;                                        /* fmt precedes data in every valid file */
        } else {
            fseek(f, (long)sz, SEEK_CUR);                 /* LIST / fact / cue / ... */
        }
...
static inline void emit_mono(float x)
{
    x *= s_emit_gain;
    float ax = fabsf(x);
    if (ax > AUD_WAV_KNEE) {                                /* soft knee: (knee..inf) -> (knee..1) */
        s_emit_limited++;
    ...
    if (s > 32767)  s = 32767;
    if (s < -32768) s = -32768;
    s_chunk[s_emit_n * 2]     = (int16_t)s;                 /* L */
    s_chunk[s_emit_n * 2 + 1] = (int16_t)s;                 /* R (duplicate — amp channel-agnostic) */
 ...
 * A 16 KB PSRAM input window is refilled from the card under short /sd locks whenever fewer than 4 KB
 * remain (a frame is <= 1441 B, and minimp3 accepts a frame only when the NEXT header is also present);
 * each decoded frame's PCM (up to 1152 samples x 2 ch, PSRAM) is downmixed through emit_mono. Tags are
 * trimmed up front — an ID3v2 block (album art can be 100s of KB the decoder would otherwise scan for
 * false syncs) is skipped by its header, and ID3v1 / APE trailers are cut off the end, because minimp3
 * validates a frame by chaining up to 10 following headers and any trailer would reject the last ~10
 * frames. Junk between frames returns 0 samples with frame_bytes > 0 (= skip). The I2S clock follows
 * the first decoded frame's rate (and is re-set if the stream changes rate). */
static void do_play_mp3(const char *path, uint16_t makeup_x100)
{
    ...
    if (f != NULL) {
        ...
        if (fseek(f, 0, SEEK_SET) == 0 && fread(t, 1, 10, f) == 10 && memcmp(t, "ID3", 3) == 0 &&
            ...
            if (t[5] & 16) start += 10;                                         /* ID3v2 footer present */
        }
    ...
    size_t   have = 0, pos = 0;
    size_t   remain = (size_t)(limit - start);             /* audio payload left to read */
    bool     eof = (remain == 0), started = false, stopped = false;
    uint32_t frames = 0, rate = 0;
    int      channels = 0;
    while (!stopped) {
        /* Refill: slide the unconsumed tail to the front, top up from the card (short lock). */
        ...
        if (have - pos == 0) break;                        /* drained */
        mp3dec_frame_info_t info;
        int n = mp3dec_decode_frame(dec, in + pos, (int)(have - pos), pcm, &info);
        if (info.frame_bytes == 0) {                       /* (defensive: this minimp3 reports the scanned span) */
            if (eof) break;
            pos = have;                                    /* force a refill */
            continue;
        }
        pos += (size_t)info.frame_bytes;
        if (n <= 0) continue;                              /* junk skipped, or a decoder resync */
    ...
    if (started) {
        emit_flush();
        play_segment(0, 6, 0);                             /* tail silence flushes the last DMA frames */
        i2s_channel_disable(s_tx);
        nocsif_power_speaker_rail(false);
    }
    s_playing = false;
...
{
    s_stop_req = false;                                    /* a stale Stop must not kill THIS play */
    if (has_ext(path, ".mp3")) {
...
static void do_play_wav_stream(const char *path, uint16_t makeup_x100)
{
    ...
    nocsif_sdcard_unlock();                                /* the handle stays valid; the lock guards the bus */
    if (!ok) {
    ...
    /* Master volume (attenuation) x make-up, in normalized [-1,1] float; emit_mono applies the soft-knee
     * limiter ((knee..inf) -> (knee..1) on a smooth 1/(1+t) curve) and the saturating clamp. */
    s_emit_gain    = ((float)makeup_x100 / 100.0f) * ((float)s_vol / 255.0f);
    s_emit_n       = 0;
    s_emit_limited = 0;
    const size_t want = (AUD_FILE_BLOCK / w.block) * w.block;    /* whole frames per read */
    ...
    while (left > 0 && !stopped) {
        ...
        nocsif_sdcard_unlock();
        if (rd < w.block) break;                           /* EOF / short file */
        rd  -= rd % w.block;
        left -= (uint32_t)rd;
...
esp_err_t nocsif_audio_init(void)
{
     ...
     * -> SDSPI) is a deep chain on this worker — headroom avoids the 4 KB-worker overflow class. Low
     * priority — cosmetic sound must never preempt the UI/radio work. Not Task-WDT-subscribed.
...
{
    /* Claim the I2S TX DMA descriptors NOW, from the pristine boot pool, BEFORE esp_wifi_init
     * fragments the internal-DMA hole. The ~3.8 KB of descriptors (4 x 240 frames x 4 B, trimmed from
     * the default 6 for the IMU margin — see i2s_lazy_open) can ONLY live in internal DMA-capable SRAM
     * (IDF hard-codes I2S_DMA_ALLOC_CAPS = INTERNAL|DMA|8BIT — the
     * peripheral GDMA reads them directly, so there is no PSRAM route like the LoRa stack has). Once
     * init succeeds the channel persists (never i2s_del_channel'd) so it is held for the session,
     * mirroring nocsif_ble_boot_reserve. Left DISABLED here (i2s_lazy_open does not enable) — no BCLK/
     * WS toggling and no amp rail powered until an actual play. Safe-mode callers skip this (keeps boot
     * light like the BLE reserve) -> tx_ready stays false -> the cue honestly shows n/a. */
...
{
    return s_tx != NULL;            /* the honest gate: true only after the channel actually claimed DMA */
}
...
static void post(const audio_cmd_t *c)
{
    ...
    xQueueSend(s_q, c, 0);          /* non-blocking; a full queue drops the request (sound is best-effort) */
}
...
void nocsif_audio_cue(nocsif_audio_cue_t cue)
{
    ...
    audio_cmd_t c = s_cues[cue];                       /* copy so we can scale by the master volume */
...
{
    if (s_boot_snd) {
        nocsif_audio_cue(NOCSIF_AUDIO_BOOT);   /* still gated by the master mute inside _cue */
    }
}
...
{
    if (s_shake_snd) {
        nocsif_audio_cue(cue);   /* still gated by the master mute inside _cue */
    }
}
...
void nocsif_audio_play_file(const char *path, uint16_t makeup_x100)
{
    ...
    if (s_playing) {
        s_stop_req = true;          /* tap-to-switch: the running file ends within a chunk, then ours plays */
    }
    audio_cmd_t c;
    memset(&c, 0, sizeof c);
    c.kind        = AUD_CMD_WAV;
