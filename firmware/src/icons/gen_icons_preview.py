...
stroke-expanded glyphs can be eyeballed before trusting them on-device. Run gen_icons.py
first to build the TTF.
...
def main():
    ...
    img = Image.new("RGB", (W, H), (7, 7, 8))          # dark ground, matches the on-panel look
    ...
    for i, (name, _sw, _svg) in enumerate(ICONS):
        ...
        # draw the glyph large (steel, rest state) and small (violet, pressed state) side by side
