 ...
 *
 * Bring-up mirrors LilyGo's proven SensorBHI260AP::initImpl() path (vendored
 * Bosch BHY2 API in components/bhy2): probe 0x28, configure the host
 * interrupt/interface, upload the ~117 KB firmware to the hub's RAM (this
 * board boots from RAM, not flash), boot it, then enable the accelerometer
 * passthrough virtual sensor and drain the FIFO.
 *
 * A dedicated worker task owns the sensor and does the slow (~2-3 s) upload
 * + FIFO polling off the LVGL task; a short spinlock guards the tiny value
 * cache the LVGL-side getters read. Polling (no GPIO8 IRQ) is deliberate for
 * this first slice — the interrupt-driven read and the wrist-wear-wakeup /
 * step-counter virtual sensors come in later M11 slices.
 */
#include "imu.h"
...
/* Set to 1 to emit a ~1 Hz accelerometer line to the log while streaming
 * (bring-up / debug aid; also surfaces in System > Diagnostics via the
 * ESP_LOG tee). Off in the shipped build — the getters
 * (nocsif_imu_accel_g / _status_str) are the real interface. */
...
/* ESP32-S3 I2C max read/write per transaction is 64 B (SensorLib's tested
 * value); the BHY2 host-interface layer chunks the firmware upload to this. */
#define IMU_MAX_RW             64
#define IMU_ACCEL_RATE_HZ      50.0f
/* F2 heading: game-rotation-vector (accel+gyro fusion, no mag). 25 Hz is
 * plenty for a hand rotation sweep and lighter than the accel rate; it only
 * runs while Signal Hunt asks for it. */
...
/* 6 KB: the firmware-upload path (bhy2 hif -> I2C driver) plus ESP_LOG
 * formatting is a moderately deep chain run thousands of times; headroom is
 * cheap and avoids the 4 KB-worker overflow class. */
#define IMU_TASK_STACK         6144
#define IMU_TASK_PRIO          3
#define IMU_FIFO_WORK_SIZE     2048
/* B1 shake-to-wake (accel-based, host-side). A deliberate wrist twist
 * back-and-forth produces large sample-to-sample accel changes; this is
 * integrated with a leaky accumulator and wakes only when the motion stays
 * vigorous long enough — so a single bump, a slow raise, or an arm swing
 * while walking doesn't trigger, only a real shake. Tunable: MOVE = the
 * per-sample L1 accel delta (g) that counts as "vigorous"; FIRE = net
 * vigorous samples needed (~20 ms each -> ~FIRE*20 ms of sustained shaking);
...
/* Stillness (M11 F1 power): the per-20 ms L1 accel delta below which the
 * sample counts as "still". Small enough that a worn wrist's micro-motion
 * usually clears it, so the watch reads "still" mainly when set down. The UI
 ...
 * getters (readers); a short spinlock keeps the read/write of a few floats
 * coherent without a heavier mutex. */
...
/* Wrist-raise auto-wake (B1). Detection is accel-based and runs on the
 * worker; the UI setter only records intent (s_wrist_want). s_wrist_events
 * is a rising-edge latch the LVGL side drains. */
...
/* The 2 KB FIFO work buffer lives in PSRAM (allocated once in
 * nocsif_imu_init). The S3's I2C peripheral has no DMA — the i2c_master
 * driver copies the hardware FIFO into this buffer from its (non-IRAM) ISR,
 * which cannot run while the flash cache is off — so a PSRAM target is
 * safe; bhy2 parses it on this worker. Frees 2,048 B of boot-permanent
 * internal BSS from the contended pool. */
static uint8_t                *s_fifo_work;
/* Decodes the I2C failures behind `fifo process err -3` (BHY2_E_IO). The new
 * i2c_master driver allocates nothing per call and the S3 I2C has no DMA, so
 * a failure here is a BUS event (timeout / NACK), not memory — log the real
 * esp_err_t next to the int-DMA gauge so the correlation can be settled
 * on-device. Rate-limited: the first IMU_I2C_ERR_LOG_FIRST failures, then
 * every IMU_I2C_ERR_LOG_EVERY-th. */
...
/* On-device root cause: the failure behind `fifo process err -3` is
 * ESP_ERR_INVALID_STATE with no i2c_master timeout log — in the driver that
 * combination is a NACK (a timeout logs ESP_LOGE; a NACK only ESP_LOGD): the
 * BHI260 refused an INT_STATUS (0x2D) read once, ~1 s after a WiFi
 * association event, with plenty of int-DMA free. Hub-side and transient —
 * the next 20 ms poll always succeeded. So: retry once after a tick, which
 * is well inside the 20 ms poll period. */
...
/* Logs an I2C failure that survived the retry, rate-limited. */
...
/* Game-rotation-vector -> tilt-robust relative heading (F2). A raw euler yaw
 * only equals a turn-heading when the watch is flat; tilted (in the hand /
 * on the wrist) it mixes tilt into the angle. Instead this rotates the
 * watch's 12-o'clock axis (body +Y) into the world frame by the fusion
 * quaternion and takes its azimuth in the horizontal plane: turning about
 * vertical moves it 1:1 at any tilt, while tilting the watch to read it
 * leaves it put. Relative only (game rotation vector has no magnetometer
 * reference), so the sweep zeroes it at start. Normalized to [0,360). */
static void imu_gamerv_cb(const struct bhy2_fifo_parse_data_info *info, void *priv)
{
    ...
    /* World X/Y components of body +Y = columns of the quaternion rotation
     * matrix (the Z part, straight up/down, is discarded by the horizontal
     * projection). */
...
/* Powers the sensor rail, probes and identifies the BHI260AP, uploads and
 * boots its RAM firmware, registers the FIFO parse callbacks, and enables
 * the accelerometer virtual sensor. Runs on the worker task. */
static esp_err_t imu_bring_up(void)
{
    ...
    /* Host interrupt control (0x07): status FIFO on, debug off, level /
     * active-high / push-pull; and enable the async status channel (0x06).
     * Matches LilyGo; sets the INT pin up for the later IRQ-driven read even
     * though this slice polls. */
    uint8_t hic = 0;
    ...
    /* Upload firmware to RAM + boot. ~117 KB over I2C takes a couple of seconds. */
    ...
    /* F2 heading: register the GAMERV parser now, but leave the sensor OFF
     * (rate 0) — the worker turns it on only when the UI requests heading,
     * so the gyro is not powered at idle. */
    ...
    /* F2 heading: the gyro must share the accel's mounting matrix, or the
     * GAMERV fusion (which combines both) gets inconsistent axes and
     * produces a wrong rotation vector. Same physical package, same map. */
    ...
    /* B1 shake-to-wake is accel-based (the detector lives in imu_task) —
     * available whenever the accelerometer streams, so mark it ready now.
     * See the SHAKE_* rationale up top. */
    taskENTER_CRITICAL(&s_lock);
    s_wrist_ready = true;
    taskEXIT_CRITICAL(&s_lock);
...
/* Worker task: brings the hub up (retrying on failure), then loops forever
 * polling the FIFO and servicing the shake/stillness/heading state machines. */
static void imu_task(void *arg)
{
    ...
    for (;;) {
...
#endif
        /* F2 heading: enable/disable the GAMERV virtual sensor to match the
         * UI's request. Done here on the worker (I2C-owning) task; toggling
         * rate 0<->IMU_HEADING_RATE_HZ powers the gyro only while a hunt
         * actually needs a bearing. */
        taskENTER_CRITICAL(&s_lock);
        ...
        /* B1 shake-to-wake detector (accel-based, worker task — never
         * touches LVGL). Integrate the per-sample accel change with a leaky
         * accumulator; a sustained vigorous wrist shake climbs it to
         * SHAKE_FIRE and latches a wake, while a single bump or slow motion
         * decays back to zero. */
        taskENTER_CRITICAL(&s_lock);
        ...
        /* Stillness tracker (always on, independent of shake): count
         * consecutive near-constant samples so the UI can sleep the panel
         * early when the watch is set down (F1 "sleep when still"). */
        if (have) {
            if (s_st_have_prev) {
...
esp_err_t nocsif_imu_init(void)
{
    ...
    if (s_fifo_work == NULL) {
        s_fifo_work = heap_caps_malloc(IMU_FIFO_WORK_SIZE, MALLOC_CAP_SPIRAM);   /* off the contended internal pool */
        if (s_fifo_work == NULL) {
            s_started = false;
    ...
    /* Stack in PSRAM: this worker only talks I2C (no DMA on the S3 I2C
     * peripheral; the driver copies its hardware FIFO from a non-IRAM ISR)
     * and never runs with the flash cache disabled (no on-task NVS/flash
     * writes), so its 6 KB no longer competes for the scarce contiguous
     * internal-DMA hole. Never deleted (the retry loop lives inside the
     * task) -> no vTaskDeleteWithCaps needed. */
