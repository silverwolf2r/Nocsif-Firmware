...
void RfalRfST25R3916Class::st25r3916ProcessInterrupts(void)
{
  ...
  /* NocSif M6-P1 patch: poll the IRQ line in SPI mode too. Upstream only polls the line for
   * I2C (`i2c_enabled && lineAsserted`) and otherwise relies on isr_pending being set by the
   * attachInterrupt ISR. NocSif drives the ST25R3916 purely by polling (no ISR — see
   * arduino_compat/Arduino.h), so without this every IRQ-sequenced op (field-on collision
   * avoidance, transceive) never sees its completion IRQ and fails (ERR_INTERNAL / ERR_IO). */
  if (!isr_pending && !lineAsserted) {
    return;
  }
